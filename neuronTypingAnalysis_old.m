...
[Spds Cohs] = meshgrid(speeds,cohs);
for uniti = 1:size(zInit,4)
    ...
    for ti = 151:size(zDyn,1)
        ...
        warning('off','all')
        Bdyn(:,uniti,ti) = regress(ztemp,[dynCoh.coh(ti-150,:)',ones(numel(ztemp),1)]);
        warning('on','all')
    end
...
BinitTi = BinitPCA(:,:,initCoh.neuron_t==75);
...
if calcCC
    ...
    subplot(2,2,4)
    plot(edges(1:end-1)+(edges(2)-edges(1))/2,p')
...
for ti = 1:size(tempR,1)
    plot(tempR(ti,1,1,pcdims(1)),tempR(ti,1,1,pcdims(2)),'o','Color',[0 0 1-tempBeta(ti,1)],...
        'MarkerFaceColor',[0 0 1-tempBeta(ti,1)])
    hold on
end
tempR = RDynlowD(indvec,5,:);
for ti = 1:size(tempR,1)
    plot(tempR(ti,1,pcdims(1)),tempR(ti,1,pcdims(2)),'o','Color',[1-tempBeta(ti,2) 0 0],...
        'MarkerFaceColor',[1-tempBeta(ti,2) 0 0])
    hold on
end
...
for ti = 1:2:size(RlowD,1)
    plot(RlowD(ti,2,2,pcdims(1)),RlowD(ti,2,2,pcdims(2)),'o','Color',[0 0 1-tempBeta2(ti,1)],...
        'MarkerFaceColor',[0 0 1-tempBeta2(ti,1)])
    hold on
end
for ti = 1:2:size(RDynlowD,1)
    if ~isnan(tempBeta2(ti,2))
        plot(RDynlowD(ti,5,pcdims(1)),RDynlowD(ti,5,pcdims(2)),'o','Color',[1-tempBeta2(ti,2) 0 0],...
            'MarkerFaceColor',[1-tempBeta2(ti,2) 0 0])
    end
end
